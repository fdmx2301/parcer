...
class ArticleFetcherTests(TestCase):
    ...
    async def test_fetch_mixed_success_failure(self, MockClientSession):
        ...
        try:
            ...
            mock_session.close()
'''
# FIXME: не дропается тестовая БД после выполнения всех тестов
class CommandTests(TestCase):
    @patch('aiohttp.ClientSession')
    def test_handle_success(self, MockClientSession):
        hub = Hub.objects.create(name='Хаб 1', url='https://example.com/hub1')
        ...
        try:
            mock_session.get.return_value.__aenter__.return_value.text = AsyncMock(return_value='<html><a href="https://example.com/hub1/article/1">Article 1</a></html>')
